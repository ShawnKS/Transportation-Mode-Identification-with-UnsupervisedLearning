...
def encoder_network(latent_dim, num_filter_ae_cls, input_combined, input_labeled, input_mixed):
    encoded_combined = input_combined
    encoded_labeled = input_labeled
    encoded_mixed = input_mixed
    layers_shape = []
    for i in range(len(num_filter_ae_cls)):
        ...
        with tf.variable_scope(scope_name, reuse=True, initializer=initializer):               
            encoded_mixed = tf.layers.conv2d(inputs=encoded_mixed, activation=tf.nn.relu, filters=num_filter_ae_cls[i],
                                               name='conv_1', kernel_size=kernel_size, strides=strides, padding=padding)     
        ...
        if i % 2 != 0:
            ...
            encoded_labeled = tf.layers.max_pooling2d(encoded_labeled, pool_size=pool_size,
                                                          strides=pool_size, name='pool')
            encoded_mixed = tf.layers.max_pooling2d(encoded_mixed, pool_size=pool_size,
                                                          strides=pool_size, name='pool')
        layers_shape.append(encoded_combined.get_shape().as_list())
    layers_shape.append(encoded_combined.get_shape().as_list())
    latent_combined = encoded_combined
    latent_labeled = encoded_labeled
    latent_mixed = encoded_mixed

    return latent_combined, latent_labeled, latent_mixed, layers_shape
...
def classifier_mlp(latent_labeled, latent_mixed, num_class, num_filter_cls, num_dense):
    conv_layer = latent_labeled
    conv_layer2 = latent_mixed
    # print(latent_labeled.shape)
    # print(latent_mixed.shape)
    # sys.exit(0)
    for i in range(len(num_filter_cls)):
        ...
        with tf.variable_scope(scope_name, reuse=tf.AUTO_REUSE, initializer=initializer):
            ...
            conv_layer2 = tf.layers.conv2d(inputs=conv_layer2, activation=tf.nn.relu, filters=num_filter_cls[i],
                                          kernel_size=kernel_size, strides=strides, padding=padding,
                                          kernel_initializer=initializer)
        if len(num_filter_cls) % 2 == 0:
            if i % 2 != 0:
                conv_layer = tf.layers.max_pooling2d(conv_layer, pool_size=pool_size,strides=pool_size, name='pool')
                conv_layer2 = tf.layers.max_pooling2d(conv_layer2, pool_size=pool_size,strides=pool_size, name='pool')
        else:
            if i % 2 == 0:
                conv_layer = tf.layers.max_pooling2d(conv_layer, pool_size=pool_size,strides=pool_size, name='pool')
                conv_layer2 = tf.layers.max_pooling2d(conv_layer2, pool_size=pool_size,strides=pool_size, name='pool')
    ...
    dense2 = tf.layers.flatten(conv_layer2)
    ...
    for i in range(num_dense):
        ...
        with tf.variable_scope(scope_name, reuse=tf.AUTO_REUSE, initializer=initializer):
            ...
            dense2 = tf.layers.dense(dense2, units, activation=tf.nn.relu, kernel_initializer=initializer)
        units /= 2
    dense_last = dense
    dense_last2 = dense2
    dense = tf.layers.dropout(dense, 0.5)
    dense2 = tf.layers.dropout(dense2,0.5)
    scope_name = 'cls_last_dense_'
    with tf.variable_scope(scope_name, reuse=tf.AUTO_REUSE, initializer=initializer):
        classifier_output = tf.layers.dense(dense, num_class, name='FC_4', kernel_initializer=initializer)
        classifier_output2 = tf.layers.dense(dense2, num_class, name='FC_4', kernel_initializer=initializer)
    return classifier_output,classifier_output2, dense_last, dense_last2

# 加上mixed部分的output


def semi_supervised(input_labeled, input_combined, input_mixed, true_label, mixed_label, alpha, beta, num_class, latent_dim, num_filter_ae_cls, num_filter_cls, num_dense, input_size):

    latent_combined, latent_labeled,latent_mixed, layers_shape = encoder_network(latent_dim=latent_dim, num_filter_ae_cls=num_filter_ae_cls,
    ...
    classifier_output,classifier_output2, dense , dense2 = classifier_mlp(latent_labeled, latent_mixed, num_class, num_filter_cls=num_filter_cls, num_dense=num_dense)
    ...
    loss_cons = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels = mixed_label, logits=classifier_output2),
                            name = 'loss_cons')
...
def loss_acc_evaluation(Test_X, Test_Y, loss_cls, accuracy_cls, input_labeled, true_label, k, sess):
    metrics = []
    batch_sizeT = 4
    for i in range(len(Test_X) // batch_sizeT):
        Test_X_batch = Test_X[i * batch_sizeT:(i + 1) * batch_sizeT]
        Test_Y_batch = Test_Y[i * batch_sizeT:(i + 1) * batch_sizeT]
    ...
    Test_X_batch = Test_X[(i + 1) * batch_sizeT:]
    Test_Y_batch = Test_Y[(i + 1) * batch_sizeT:]
...
def training(one_fold, X_unlabeled, seed, prop, num_filter_ae_cls_all, epochs_ae_cls=20):
    ...
    num_filter_ae_cls_all = [[114,114,228,228,456,456]]
    class_posterior = []
    ...
    for z in range(len(num_filter_ae_cls_all)):
        ...
        with tf.Session() as sess:
            ...
            input_mixed = tf.placeholder(dtype=tf.float32, shape=[None] + input_size, name='input_mixed')
            true_label = tf.placeholder(tf.float32, shape=[None, num_class], name='true_label')
            mixed_label = tf.placeholder(tf.float32, shape=[None, num_class], name='mixed_label')
            ...
            for k in range(epochs_ae_cls):
                ...
                print('====================================================')
                saver.save(sess, "/home/sxz/cnv-TF/"  + str(z) + '/' + str(prop), global_step=k)
                ...
                if all([change_to_ae, val_accuracy[k] < val_accuracy[k - 1], val_accuracy[k] < val_accuracy[k - 2]]):
                    ...
                    save_path = "/home/sxz/cnv-TF/"  + str(z) + '/' + str(prop) + '-' + str(max_acc)
...
def training_all_folds(label_proportions, num_filter):
    ...
    for index, prop in enumerate(label_proportions):
        print('All Test Accuracy For Multi with Prop {} are: {}'.format(prop, test_accuracy_fold[index]))
